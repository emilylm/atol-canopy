...
class ReadService(BaseService[Read, ReadCreate, ReadUpdate]):
    ...
    def get_by_read_id_serial(self, db: Session, read_id_serial: str) -> Optional[Read]:
        """Get read by read ID serial."""
        return db.query(Read).filter(Read.read_id_serial == read_id_serial).first()
